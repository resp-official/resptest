    ...
    return (
        <div className="relative w-full h-[700px] flex items-center justify-center overflow-hidden rounded-[3rem] bg-[#02040a] shadow-[0_0_150px_rgba(0,0,0,1)] border border-white/5">
            <div className="absolute inset-0 z-0 overflow-hidden">
                ...
                {[...Array(80)].map((_, i) => (
                    <motion.div key={i} animate={{ opacity: [0.1, 0.8, 0.1] }} transition={{ duration: Math.random() * 4 + 2, repeat: Infinity }}
                        className="absolute w-[1.5px] h-[1.5px] bg-white rounded-full"
                        style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, boxShadow: '0 0 4px rgba(255,255,255,0.6)' }}
                    />
                ))}
            </div>
            ...
            <motion.div animate={{ 
            ...
            }} transition={{ duration: 8, ease: [0.4, 0, 0.2, 1] }} className="relative w-full h-full flex items-center justify-center">
                <motion.div animate={{ opacity: stage === 'extraction' ? 0 : 1 }} className="absolute z-10">
                    <div className="relative w-[1150px] h-[1150px] rounded-full shadow-[inset_-120px_-120px_220px_rgba(0,0,0,0.98)] overflow-hidden">
                        <motion.div animate={{ backgroundPositionX: ['0%', '100%'] }} transition={{ duration: 120, repeat: Infinity, ease: "linear" }}
                            className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1614730321146-b6fa6a46bcb4?q=80&w=1974&auto=format&fit=crop')] bg-cover opacity-95"
                ...
                }} transition={{ duration: 8 }} className="absolute z-30 flex flex-col items-center">
                    <div className="relative w-80 h-40 bg-[#1e293b] rounded-2xl border-l-[20px] border-[#475569] overflow-hidden shadow-2xl">
                        <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-black/70" />
                        {stage === 'extraction' && (
                            <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} className="absolute top-20 left-72 w-6 h-[650px] bg-gradient-to-b from-purple-700 via-white to-cyan-200 origin-top z-40" />
                        )}
                    </div>
                </motion.div>

                <motion.div animate={{ opacity: stage === 'launch' ? 0 : 1, x: 1450 }} transition={{ duration: 8 }} className="absolute z-20">
                    <div className="relative w-[1500px] h-[1500px] rounded-full bg-[#cbd5e1] shadow-[inset_-140px_-140px_300px_rgba(0,0,0,1)] overflow-hidden">
                        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1522030239044-1230198d0bc7?q=80&w=1974&auto=format&fit=crop')] bg-cover opacity-98" />
                    </div>
                </motion.div>
            </motion.div>
            ...
            <div className="absolute inset-0 z-50 pointer-events-none flex flex-col justify-center px-16 md:px-24">
                <div className="max-w-4xl text-left">
                    <motion.h1 initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="text-4xl md:text-6xl font-display font-bold leading-[1.1] tracking-tighter mb-8 drop-shadow-2xl">
                        Beyond Earth.<br />
                    ...
                    <motion.p initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} className="text-lg md:text-xl text-slate-100/90 max-w-xl font-light">
            ...
            <div className="absolute bottom-16 left-16 md:left-24 z-50 pointer-events-none">
                <div className="flex items-center gap-5">
                    ...
                    <span className="text-[13px] font-mono text-white/95 uppercase tracking-[0.6em] font-bold notranslate" translate="no">
...
const App: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen selection:bg-purple-500/30">
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-xl py-3' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <a href="#" className="flex items-center gap-6 group">
            <div className="w-10 h-10 text-white transition-transform group-hover:scale-110"><RespLogo /></div>
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Roadmap', 'NFT Launch', 'Contact'].map(item => (
              <a key={item} href={`#${item.toLowerCase().replace(' ', '-')}`} onClick={(e) => handleNavClick(e, item.toLowerCase().replace(' ', '-'))} className="text-sm font-medium text-slate-400 hover:text-white transition-colors uppercase tracking-widest text-[10px]">{item}</a>
            ))}
          </div>
        </div>
      </nav>
      ...
      <section className="relative pt-32 pb-20 px-6 min-h-[80vh] flex flex-col justify-center">
        <div className="max-w-7xl mx-auto w-full flex flex-col gap-12">
          ...
          <div className="flex flex-col sm:flex-row gap-8 px-16 md:px-24 items-center">
              <button onClick={() => setIsMissionOpen(true)} className="bg-purple-600 hover:bg-purple-500 text-white px-12 py-5 rounded-3xl font-bold flex items-center gap-3 transition-all uppercase text-sm tracking-[0.2em]">Missions <ArrowRight /></button>
          ...
          <div className="flex flex-col lg:flex-row gap-20 items-start">
            <div className="w-full lg:w-1/2 text-left">
              ...
              <form onSubmit={handleScan} className="relative group">
                ...
                <button type="submit" disabled={isScanning} className="absolute right-3 top-3 bottom-3 bg-cyan-500 text-black px-8 rounded-2xl font-bold">{isScanning ? <Loader2 className="animate-spin" /> : <Search />}</button>
              </form>
            </div>
            <div className="w-full lg:flex-1 min-h-[500px]">
              <AnimatePresence mode="wait">
                {isScanning ? (
                  <motion.div key="scanning" className="glass-panel h-full rounded-[3rem] flex flex-col items-center justify-center p-20"><Activity className="w-16 h-16 text-cyan-400 animate-pulse" /><p className="mt-8 text-cyan-400 font-mono text-sm tracking-[0.4em] uppercase">Cross-referencing Global Datasets...</p></motion.div>
                ) : scanResult ? (
                  <motion.div key="result" className="glass-panel rounded-[3rem] p-12 text-left shadow-[0_0_100px_rgba(34,211,238,0.1)]">
                    ...
                    <div className="grid grid-cols-2 gap-8">{scanResult.composition.map((comp, i) => (<div key={i}><div className="flex justify-between text-[11px] font-bold uppercase"><span className="text-white">{comp.material}</span><span className="text-cyan-400">{comp.percentage}%</span></div><div className="h-1 bg-white/5 rounded-full overflow-hidden mt-2"><motion.div initial={{ width: 0 }} animate={{ width: `${comp.percentage}%` }} className="h-full bg-cyan-500" /></div></div>))}</div>
                  </motion.div>
                ) : <div className="glass-panel h-full rounded-[3rem] flex flex-col items-center justify-center p-20"><Target className="w-16 h-16 text-slate-800 mb-8" /><p className="text-slate-600 font-display uppercase tracking-[0.3em] text-sm">Awaiting Target Designation...</p></div>}
              </AnimatePresence>
            </div>
          </div>
        </div>
      ...
      <section id="roadmap" className="py-32 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
            {roadmap.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="glass-panel p-8 rounded-3xl text-left border-purple-500/10">
      ...
      <section id="nft-launch" className="py-32 px-6 bg-white/[0.01]">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {nftTiers.map((nft, i) => (
              <a key={i} href={nft.link} target="_blank" rel="noopener" className={`glass-panel p-10 rounded-[2.5rem] transition-all hover:scale-[1.02] flex flex-col items-center group ${nft.featured ? 'border-purple-500/40 bg-purple-500/5' : 'border-white/5'}`}>
                <div className="text-5xl mb-6 group-hover:scale-125 transition-transform">{nft.icon}</div>
                <h4 className="text-white font-bold mb-4 uppercase tracking-widest text-xs">{nft.tier}</h4>
                <span className="text-slate-400 text-[10px] group-hover:text-purple-400 transition-colors uppercase font-mono font-bold">Buy on Rarible</span>
              </a>
            ))}
          </div>
        </div>
      ...
      {/* Missions Modal Content Restored */}
      <AnimatePresence>
        {isMissionOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 md:p-20">
            <div className="absolute inset-0 bg-black/80 backdrop-blur-3xl" onClick={() => setIsMissionOpen(false)} />
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-6xl glass-panel border-purple-500/30 rounded-[3rem] p-12 overflow-hidden shadow-2xl">
                ...
                <div className="text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                        <div className="glass-panel p-8 rounded-3xl border-purple-500/20">
                            <Rocket className="w-8 h-8 text-purple-400 mb-6" />
                            <h4 className="text-white font-bold mb-4 uppercase text-xs tracking-widest">Operation: OSIRIS-01</h4>
                            <p className="text-slate-400 text-sm leading-relaxed">The first lunar surface extraction prototype. Testing Core Drill-X and low-gravity stabilization systems.</p>
                        </div>
                        <div className="glass-panel p-8 rounded-3xl border-cyan-500/20">
                            <Database className="w-8 h-8 text-cyan-400 mb-6" />
                            <h4 className="text-white font-bold mb-4 uppercase text-xs tracking-widest">Protocol: SPECTRA</h4>
                            <p className="text-slate-400 text-sm leading-relaxed">Live spectroscopic analysis of near-Earth asteroids using the AI Core. Target identification and value mapping.</p>
                        </div>
                        <div className="glass-panel p-8 rounded-3xl border-blue-500/20">
                            <Shield className="w-8 h-8 text-blue-400 mb-6" />
                            <h4 className="text-white font-bold mb-4 uppercase text-xs tracking-widest">Civ: RESP-DAO</h4>
                            <p className="text-slate-400 text-sm leading-relaxed">Decentralized governance for interplanetary assets. Community voting on extraction locations and resource sharing.</p>
                        </div>
                    </div>
                </div>
            </motion.div>
